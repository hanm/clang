...
class [[asap::param("P")]] SetYFunctor {
  Point &P;  // expected-warning{{Inferred region arguments: class Point &, IN:<empty>, ArgV:[p5_P]}}  
...
class //[[asap::region("Rx,Ry")]]
       Point {
  int x;  // expected-warning{{Inferred region arguments: int, IN:[r8_x], ArgV:}}
  int y;  // expected-warning{{Inferred region arguments: int, IN:[r9_y], ArgV:}}
 public:
  ...
  void setXY(int _x, int _y) { // expected-warning{{Inferred Effect Summary for setXY: [reads(rpl([r1_v],[])),reads(rpl([r5_v],[])),reads(rpl([rLOCAL],[])),writes(rpl([r8_x],[])),writes(rpl([r9_y],[]))]}}
    SetXFunctor SXF(*this, _x); // expected-warning{{Inferred region arguments: class SetXFunctor, IN:<empty>, ArgV:[p6_Point]}}
    SetYFunctor SYF(*this, _y); // expected-warning{{Inferred region arguments: class SetYFunctor, IN:<empty>, ArgV:[p6_Point]}}
