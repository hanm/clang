...
class [[asap::param("P")]] SetXFunctor {
  Point &P; // expected-warning{{Inferred region arguments: class Point &, IN:<empty>, ArgV:[p6_P]}}
  int v; // expected-warning{{Inferred region arguments: int, IN:[r1_v], ArgV:}}
public:
  SetXFunctor(Point &P[[asap::arg("P")]], int v) : P(P), v(v) {} // expected-warning{{Inferred Effect Summary for SetXFunctor: [reads(rpl([rLOCAL],[]))]}}
  SetXFunctor(SetXFunctor &F) = delete; // expected-warning{{Inferred region arguments: class SetXFunctor &, IN:<empty>, ArgV:[r2_F]}}
  SetXFunctor(SetXFunctor &&F) = delete; // expected-warning{{Inferred region arguments: class SetXFunctor &&, IN:<empty>, ArgV:[r3_F]}}
  void operator() () const;
...
public:
  ...
  SetYFunctor(SetYFunctor &&F) = delete; // expected-warning{{Inferred region arguments: class SetYFunctor &&, IN:<empty>, ArgV:[r7_F]}}

  void operator() () const;
}; // end class SetYFunctor

class [[asap::param("P")]] SetXYFunctor {
  Point &P; // expected-warning{{Inferred region arguments: class Point &, IN:<empty>, ArgV:[p8_P]}}
  int v1; // expected-warning{{Inferred region arguments: int, IN:[r9_v1], ArgV:}}
  int v2; // expected-warning{{Inferred region arguments: int, IN:[r10_v2], ArgV:}}
...
public:
  ...
  SetXYFunctor(SetXYFunctor &F) = delete; // expected-warning{{Inferred region arguments: class SetXYFunctor &, IN:<empty>, ArgV:[r11_F]}}
  SetXYFunctor(SetXYFunctor &&F) = delete; // expected-warning{{Inferred region arguments: class SetXYFunctor &&, IN:<empty>, ArgV:[r12_F]}}
...
class //[[asap::region("Rx,Ry")]]
       Point {
  int x;  // expected-warning{{Inferred region arguments: int, IN:[p9_Point], ArgV:}}
  int y;  // expected-warning{{Inferred region arguments: int, IN:[p9_Point,r14_y], ArgV:}}
 public:
  ...
  Point() : x(0), y(0) {}
  Point(Point &P) = delete; // expected-warning{{Inferred region arguments: class Point &, IN:<empty>, ArgV:[r17_P]}}
  Point(Point &&P) = delete; // expected-warning{{Inferred region arguments: class Point &&, IN:<empty>, ArgV:[r18_P]}}
};
