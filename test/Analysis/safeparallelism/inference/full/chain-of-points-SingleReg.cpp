...
struct //[[asap::param("Pl")]] 
    ...
    link *next; // expected-warning{{Inferred region arguments: struct link *, IN:[r4_next], ArgV:[rGLOBAL]}}

    //[[asap::param("Q"), asap::reads("Q")]]
    link(const point &pos_in /*[[asap::arg("Pl")]]*/) // expected-warning{{Inferred region arguments: const class point &, IN:<empty>, ArgV:[rGLOBAL]}}
        :
        pos(pos_in),
        next(nullptr) {}
    // copy constructor
    ...
    link &operator = /*[[asap::arg("Pl")]]*/ // expected-warning{{Inferred region arguments: struct link &(struct link &&), IN:<empty>, ArgV:[r8_operatorAssign]}}
                     (link && l/*[[asap::arg("Pl")]]*/) { // expected-warning{{Inferred region arguments: struct link &&, IN:<empty>, ArgV:[r8_operatorAssign]}}
        return l;
    }
...
class //[[asap::param("P")]] 
    chain {
    link *start;  // expected-warning{{Inferred region arguments: struct link *, IN:[r10_start], ArgV:[rGLOBAL]}}
public:
    chain() : start(nullptr) {}
    ...
    int n_points /*[[asap::reads("P")]]*/ () const
        ...
        link *lp /*[[asap::arg("Local, P")]]*/ = start; // expected-warning{{Inferred region arguments: struct link *, IN:[r15_lp], ArgV:[rGLOBAL]}}
    ...
    bool split /*[[asap::writes("P")]]*/ (int n, chain &rest /*[[asap::arg("P")]]*/) {//expected-warning{{Inferred Effect Summary for split: [writes(rpl([r10_start],[])),writes(rpl([r18_lp],[])),writes(rpl([r4_next],[])),writes(rpl([rLOCAL],[]))]}}  // expected-warning{{Inferred region arguments: class chain &, IN:<empty>, ArgV:[r17_rest]}}
        link *lp/*[[asap::arg("Local, P")]]*/ = start;  // expected-warning{{Inferred region arguments: struct link *, IN:[r18_lp], ArgV:[rGLOBAL]}}
