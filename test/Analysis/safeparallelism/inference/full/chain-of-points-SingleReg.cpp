...
class //[[asap::param("class")]] 
      point  {
    double m_x; // expected-warning{{Infered region arguments: double, IN:[r0_m_x], ArgV:}}
    double m_y; // expected-warning{{Infered region arguments: double, IN:[r1_m_y], ArgV:}}
...
public:
    ...
    point &operator= //[[asap::arg("class"), asap::param("P"), asap::reads("P"), asap::writes("class")]]  // expected-warning{{Infered region arguments: class point &(const class point &), IN:<empty>, ArgV:[p0_point]}}
                     [[asap::param("P")]] 
...
struct //[[asap::param("Pl")]] 
       link {
    point pos;  // expected-warning{{Infered region arguments: class point, IN:<empty>, ArgV:[r3_pos]}}
    link *next; // expected-warning{{Infered region arguments: struct link *, IN:[r4_next], ArgV:[rSTAR]}}

    //[[asap::param("Q"), asap::reads("Q")]]
    link(const point &pos_in /*[[asap::arg("Pl")]]*/) // expected-warning{{Infered region arguments: const class point &, IN:<empty>, ArgV:[rSTAR]}}
        :
        pos(pos_in),
        next(nullptr) {}
    // copy constructor
    link(link &l /*[[asap::arg("Pl")]]*/) : pos(l.pos), next(l.next) {} //expected-warning{{Inferred Effect Summary for link: [reads(rpl([r4_next],[]))]}} // expected-warning{{Infered region arguments: struct link &, IN:<empty>, ArgV:[r7_l]}}
    // move "constructor"
    link &operator = /*[[asap::arg("Pl")]]*/ (link && l/*[[asap::arg("Pl")]]*/) { return l; }  // expected-warning{{Infered region arguments: struct link &(struct link &&), IN:<empty>, ArgV:[rSTAR]}}  // expected-warning{{Infered region arguments: struct link &&, IN:<empty>, ArgV:[r9_l]}}
    };
...
class //[[asap::param("P")]] 
    chain {
    link *start;  // expected-warning{{Infered region arguments: struct link *, IN:[r10_start], ArgV:[rSTAR]}}
public:
    chain() : start(nullptr) {}
    ...
    void add_link /*[[asap::writes("P")]]*/ (const point &pos /*[[asap::arg("P")]]*/)  // expected-warning{{Infered region arguments: const class point &, IN:<empty>, ArgV:[r12_pos]}}
        ...
        link *new_start /*[[asap::arg("P, P")]]*/ = new link(pos);  // expected-warning{{Infered region arguments: struct link *, IN:[r13_new_start], ArgV:[r14_new_start]}}
    ...
    int n_points /*[[asap::reads("P")]]*/ () const
        ...
        link *lp /*[[asap::arg("Local, P")]]*/ = start; // expected-warning{{Infered region arguments: struct link *, IN:[r15_lp], ArgV:[rSTAR]}}
    ...
    bool split /*[[asap::writes("P")]]*/ (int n, chain &rest /*[[asap::arg("P")]]*/) {//expected-warning{{Inferred Effect Summary for split: [writes(rpl([r10_start],[])),writes(rpl([r18_lp],[])),writes(rpl([r4_next],[])),writes(rpl([rLOCAL],[]))]}}  // expected-warning{{Infered region arguments: class chain &, IN:<empty>, ArgV:[r17_rest]}}
        link *lp/*[[asap::arg("Local, P")]]*/ = start;  // expected-warning{{Infered region arguments: struct link *, IN:[r18_lp], ArgV:[rSTAR]}}
